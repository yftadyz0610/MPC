...
def setup_seed(seed):
    ...
    np.random.seed(seed)
    random.seed(seed)
    #torch.backends.cudnn.deterministic = True   # Setting True will slow down training.


# 设置随机数种子
setup_seed(6)  # seed for three runs [6, 15, 20]

class Config:
    root_dir= './'
    # training file
    file_path = 'Data/train.v6.4.round4.csv' # path of trainset
    task_name = 'cmpt_adj_attr'
    version = '6.4.1'
    epoch_num=20
    warmup=10 # warmup steps
    lr = 2e-5
    stepLR=[10] # after some steps, the learning rate would drop according to a specific factor
    max_len = 256 # input text max length
    train_bs = 32 # train batch size
    valid_bs = 64 # valid batch size
    train_pcent = 0.8 # percentage of samples used for training. The others are used for validating.
    num_workers = 0 # number of threads
    device=0 # cuda id for GPU training

    bert_model_name = 'bert-base-uncased'
    bert_model = BertModel.from_pretrained(bert_model_name, torchscript=True)
    tokenizer = BertTokenizer.from_pretrained(bert_model_name)

    # If resume training from last checkpoint, please set `resume_training` as True and assign correct version and ckpt.
    resume_training=False
    last_version=''
    last_load_ckpt_path = ''

    # Do prediction/inference please set `is_predict` as True.
    is_predict=True # if True do only predict otherwise do only train
    # Input file for making prediction.
    test_file_path='Data/adj.csv'
    # Path of loading model.
    load_ckpt_path = 'CkptSaveDir/cmpt_adj_attr/version_6.4.1/epoch=8-step=44.ckpt'
    # Path of saving prediction result.
    predict_output_path='Data/pred.v6.4.1.round4'
...
if __name__=="__main__":
    if not Config.is_predict:
        ...
        # tensorboard logger
        logger = TensorBoardLogger(save_dir=os.path.join(Config.root_dir, 'TbDir'), name=Config.task_name, version=Config.version)
        # Shell Command: tensorboard --logdir=TbDir

        ckpt_save_path = 'CkptSaveDir/' + '%s/' % Config.task_name + 'version_%s' % Config.version
        # checkpoint setting
        checkpoint_callback = ModelCheckpoint(dirpath=os.path.join(Config.root_dir, ckpt_save_path), save_top_k=1,
        ...
        trainer.fit(model)
        print("Tensorboard check command: tensorboard --logdir=%s"%os.path.join(Config.root_dir, 'tb_dir'))

        print("Ckpt is saved at: %s"%os.path.join(Config.root_dir,ckpt_save_path))

    else: # inference
        dl=set_dataloader(Config.test_file_path)
        ...
        joblib.dump(predictions,Config.predict_output_path) # the order of predictions is exactly the same as the order
        # of input samples
